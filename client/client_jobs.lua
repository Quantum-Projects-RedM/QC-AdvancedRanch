...
CreateThread(function()
    exports['rsg-target']:AddTargetModel(Config.FertilizerProps, {
        options = {
            {
                type = "client",
                event = 'qc-advancedranch:client:collectpoo',
                icon = "far fa-eye",
                label = 'Pickup Poo',
                distance = 2.0
            }
...
-- do collecting poo
RegisterNetEvent('qc-advancedranch:client:collectpoo')
AddEventHandler('qc-advancedranch:client:collectpoo', function()
    ...
    if (PlayerJob == 'macfarranch') or (PlayerJob == 'prongranch') then
        ...
        if lib.progressCircle({
            ...
            if coords then
                RSGCore.Functions.TriggerCallback('qc-advancedranch:server:checkcollectedpoop', function(exists)
                    if not exists then
                        DeleteEntity(pooObject)
                        SetObjectAsNoLongerNeeded(pooObject)
                        TriggerServerEvent('qc-advancedranch:server:collectedpoop', coords)
                        TriggerServerEvent('qc-advancedranch:server:collectjobproduct', PlayerJob, 'fertilizer', 1)
